...
namespace MVZ2.GameContent.Bosses
{
    public partial class TheGiant
    {
        ...
        private class VomitState : EntityStateMachineState
        {
            ...
            public override void OnUpdateAI(EntityStateMachine stateMachine, Entity entity)
            {
                ...
                switch (substate)
                {
                    case SUBSTATE_START:
                        if (substateTimer.Expired)
                        {
                            stateMachine.StartSubState(entity, SUBSTATE_VOMIT);
                    ...
                    case SUBSTATE_VOMIT:
                        if (substateTimer.PassedInterval(2))
                        {
                            ...
                            var grids = entity.Level.GetAllGrids().Where(g => g.Column > 0 && g.Column < 7 && !lawnGrids.Contains(g)).RandomTake(2, entity.RNG);
            ...
            public const int SUBSTATE_END = 2;
            private static readonly List<LawnGrid> lawnGrids = new List<LawnGrid>();
        }
            ...
            public override void OnUpdateAI(EntityStateMachine stateMachine, Entity entity)
            {
                ...
                switch (substate)
                {
                    case SUBSTATE_START:
                        if (substateTimer.Expired)
                        {
                            stateMachine.StartSubState(entity, SUBSTATE_SNEEZE);
                            substateTimer.ResetTime(5);
                        }
                        break;
                        ...
                        if (substateTimer.Expired)
                        {
                            ...
                            for (int lane = 0; lane < entity.Level.GetMaxLaneCount(); lane++)
                            {
                                for (int i = 0; i < count; i++)
                                {
                                    ...
                                    var y = entity.Level.GetGroundY(x, z);
                                    var param = entity.GetSpawnParams();
                                    entity.Spawn(VanillaEnemyID.zombie, new Vector3(x, y, z), param)?.Let(e =>
                                    {
                                        e.SetFlipX(entity.IsFlipX());
                                        e.Velocity += (i + 3) * 10 * entity.GetFacingDirection();
                                    });
                                }
                            }
                            stateMachine.StartSubState(entity, SUBSTATE_END);
