...
namespace MVZ2.GameContent.Bosses
{
    public partial class Wither
    {
                    ...
                    case SUBSTATE_DASH:
                        {
                            ...
                            if (reachEnd)
                            {
                                ...
                                entity.Level.Spawn(VanillaEnemyID.hellChariot, new Vector3(spawnX, 0, spawnZ), entity)?.Let(e =>
                                {
                                    Explosion.Spawn(e, e.GetCenter(), 120);
                                });
                            }
                            entity.Position = pos;
                            entity.Velocity = vel;
        ...
        {
            public MagicState() : base(STATE_MAGIC, ANIMATION_STATE_MAGIC) { }
            public override void OnEnter(EntityStateMachine stateMachine, Entity entity)
            {
                base.OnEnter(stateMachine, entity);
                var substateTimer = stateMachine.GetSubStateTimer(entity);
                                    ...
                                    case MAGIC_BERSERKER:
                                        {
                                            ...
                                            foreach (var target in targets)
                                            {
                                                var buff = target.GetFirstBuff<DetonatedBuff>();
                                                buff ??= target.AddBuff<DetonatedBuff>();
                                                buff.SetProperty(DetonatedBuff.PROP_FACTION, entity.GetFaction());
                                            }
        ...
        public const int ANIMATION_STATE_MAGIC = 8;
    }
}
