...
namespace MVZ2.GameContent.Contraptions
{
    ...
    public class TeslaCoil : ContraptionBehaviour
    {
        ...
        public static void Shock(Entity source, float damage, int faction, float shockRadius, Vector3 targetPosition, DamageEffectList? damageEffects = null, bool noChain = true)
        {
            ...
            {
                var output = collider.TakeDamage(damage, damageEffects, source);
                if (output == null)
                    continue;
                var insulator = output.GetAllResults().Any(e => e?.ShellDefinition?.IsInsulator() ?? false);
                if (!noChain && !insulator)
                {
                    var entity = collider.Entity;
                    var eBuff = entity.NewBuff<ElectricChainBuff>();
                    eBuff.SetProperty(ElectricChainBuff.PROP_DAMAGE, damage * (1 - ElectricChainBuff.DMG_DECAY));
                    eBuff.SetProperty(ElectricChainBuff.PROP_FACTION, source.GetFaction());
            ...
            if (playSound)
            {
                source.PlaySound(VanillaSoundID.redLightning);
            }
        }
        public static void CreateArc(Entity source, Vector3 sourcePosition, Vector3 targetPosition, int point = 20, int timeout = 30)
        {
            source.Spawn(VanillaEffectID.electricArc, sourcePosition)?.Let(e =>
            {
                ...
                ElectricArc.SetPointCount(e, point);
        ...
        private void UpdateElectrify(Entity coil)
        {
            ...
            {
                target.TeslaCoilElectrify(coil);
            }
        }
