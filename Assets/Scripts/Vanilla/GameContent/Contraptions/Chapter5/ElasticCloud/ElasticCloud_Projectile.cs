    ...
    public class ElasticCloud_Projectile : EntityBehaviourDefinition
    {
        ...
        private void PreProjectileHitCallback(VanillaLevelCallbacks.PreProjectileHitParams param, CallbackResult result)
        {
            ...
            if (!target.IsHostile(proj)) // ʵ����䵯�����ǵжԵ�
                return;
            // �����䵯��������Ӫ��Ϊ��ʵ�����Ӫ��
            var push = ElasticCloud.GetKnockBackMultipiler(target);
            var cost = ElasticCloud.GetHealthCostMultipiler(target);
            var velocity = target.GetFacingDirection() * proj.Velocity.magnitude * push;
            proj.Velocity = velocity;
            proj.SetFaction(target.GetFaction());
            target.TakeDamage(BOUNCE_DAMAGE * cost, new DamageEffectList(), target);
