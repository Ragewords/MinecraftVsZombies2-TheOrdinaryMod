...
namespace MVZ2.GameContent.Contraptions
{
    ...
    public class SkywardBeacon : AIEntityBehaviour
    {
        ...
        public override void PostDeath(Entity entity, DeathInfo deathInfo)
        {
            ...
            if (grid == null)
                return;
            if (!grid.CanSpawnEntity(VanillaContraptionID.beacon))
                return;
            entity.Spawn(VanillaContraptionID.beacon, entity.Position);
        }
        ...
        private void UpdateFamiliar(Entity entity)
        {
            var level = entity.Level;
            var familiarID = GetFamiliarEntity(entity);
            var familiar = familiarID?.GetEntity(level);
            if (!familiar.ExistsAndAlive())
            {
                familiar = entity.SpawnWithParams(VanillaContraptionID.skywardOption, entity.Position)?.Let(e =>
                {
                    e.SetParent(entity);
                    e.SetModelProperty("Dest", entity.GetCenter());
                    SkywardOption.SetTargetColumn(e, entity.GetColumn());
                    SkywardOption.SetTargetLane(e, entity.GetLane());
                });
        ...
        public static EntityID? GetFamiliarEntity(Entity entity) => entity.GetBehaviourField<EntityID>(PROP_FAMILIAR);
        public static void SetFamiliarEntity(Entity entity, EntityID? value) => entity.SetBehaviourField(PROP_FAMILIAR, value);
        ...
        public static readonly VanillaEntityPropertyMeta<EntityID> PROP_FAMILIAR = new VanillaEntityPropertyMeta<EntityID>("familiar");
