...
namespace MVZ2.Vanilla.Enemies
{
    public partial class PopCaptain : EnemyBehaviour
    {
        ...
        private static bool CheckSmash(Entity zombie)
        {
            if (NoAnchor(zombie))
                return false;
            return smashDetector.DetectExists(zombie) || smashDetectorFriendly.DetectExists(zombie);
        }
        ...
        public static void SmashFriendly(Entity entity, float ySpeed)
        {
            // 针对友方怪物的砸击。
            if (NoAnchor(entity))
                return;
            // 检测第一个目标。
            var center = entity.GetCenter();
            var targetCollider = damageDetectorFriendly.DetectWithTheLeast(entity, c => c.GetBoundingBox().SqrDistance(center));
            // 对目标造成伤害。
            if (targetCollider == null)
                return;
            var target = targetCollider.Entity;
            ...
            if (target.Type == EntityTypes.ENEMY)
            {
                ...
                target.Velocity += vel * knockbackMultiplier;
            }
            // 将目标眩晕，略过爆破骨兵。
            if (target.CanDeactive())
            {
