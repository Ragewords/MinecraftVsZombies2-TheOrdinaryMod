...
{
    [AutoShellDefinition(VanillaShellNames.fire)]
    public class FireShell : ShellDefinition
    {
        public FireShell(string nsp, string name) : base(nsp, name)
        {
            SetProperty(VanillaShellProps.HIT_SOUND, VanillaSoundID.fire);
        ...
        public override void EvaluateDamage(DamageInput damageInfo)
        {
            ...
            if (damageInfo.HasEffect(VanillaDamageEffects.ICE))
            {
                damageInfo.Multiply(3f);
            }
        }
        private void PreBodyTakeDamageCallback(VanillaLevelCallbacks.PreBodyTakeDamageParams param, CallbackResult result)
        {
            ...
            if (shell != this || !input.HasEffect(VanillaDamageEffects.FIRE))
                return;
