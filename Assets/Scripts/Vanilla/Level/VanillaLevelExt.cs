...
namespace MVZ2.Vanilla.Level
{
    public static partial class VanillaLevelExt
    {
        ...
        public static Entity? SpawnEnemyAtRandomLane(this LevelEngine level, SpawnDefinition spawnDef, bool ignoreOffset = false)
        {
            var lane = spawnDef.GetRandomSpawnLane(level);
            return level.SpawnEnemy(spawnDef, lane, ignoreOffset);
        }
        public static Entity? SpawnEnemyAtRandomLane(this LevelEngine level, NamespaceID spawnID, bool ignoreOffset = false)
        {
            var spawnDef = level.Content.GetSpawnDefinition(spawnID);
            if (spawnDef == null)
                return null;
            return level.SpawnEnemyAtRandomLane(spawnDef, ignoreOffset);
        }
        ...
        public static Entity? SpawnEnemy(this LevelEngine level, SpawnDefinition spawnDef, int lane, float x, bool ignoreOffset = false)
        {
            ...
            var entityID = spawnDef.GetSpawnEntityID();
            var offset = Vector3.zero;
            if (!ignoreOffset)
            {
                var def = level.Content.GetEntityDefinition(entityID);
                if (def != null)
                {
                    offset = def.GetStartingPositionOffset();
                }
            }
            return level.Spawn(entityID, pos + offset, null)?.Let(e =>
            {
