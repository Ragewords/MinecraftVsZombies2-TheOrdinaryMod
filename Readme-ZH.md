...
- 关卡中生成更多怪物。
- 部分关卡的第一次出怪时间延长。
- 已经通关的关卡通关后会给予三个钻石，没有通关的关卡通关后会给予一个钻石。
...
- 科学怪人一开始便会进入钢铁形态，且二倍速行动。
- 科学怪人在生命值降低至50%后，其头部会分离并独立行动。
- 瘦长鬼影的命运选择最多有6次，且只有1个选项。
- 瘦长鬼影会将器械蓝图变为错误方块或僵尸，且僵尸蓝图不会变成其他卡牌。
- 梦魇收割者的碾压墙速度变快。
- 梦魇收割者的时间限制为40秒。
- 梦魇收割者的“死亡之轮”攻击额外造成40%伤害。
- 被凋灵头颅伤害的器械会受到凋零效果，每秒失去10生命，持续30秒。
...
- 红龙在二阶段的巨型火球飞行和爆炸更快。
- 红龙在二阶段会召唤3道火焰龙卷风。
- 红龙在二阶段的飞空的火焰爆炸范围半径翻3倍。
- 上锁的箱子进入二阶段后，材质变为金属，被燧石击中时会生成敌方的火焰。
- 上锁的箱子进入二阶段后可以继续使用一阶段的技能。
- 上锁的箱子喷吐的蓝图为钻石盔僵尸。
- 上锁的箱子在勒索弹窗中会索要2个星之碎片。
